...
{
    int err;
    FFStream *ffs = ffstream(ist);

    if (ffs->avctx && (
        ffs->avctx->height ||
        ffs->avctx->sample_rate
    )) {
        err = avcodec_parameters_from_context(ist->codecpar, ffs->avctx);
        if (err < 0)
            return err;
    }
...
//PLEX
         || (ffs->avctx && (
              (ffs->avctx->height && !ist->codecpar->height) ||
              (ffs->avctx->sample_rate && !ist->codecpar->sample_rate)
            ))
