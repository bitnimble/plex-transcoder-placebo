...
{
    *nb_entries = st->internal->nb_index_entries;
                            ...
                            const AVPacket *avpkt, AVDictionary **options)
{
    ...
    if (st->internal->info->found_decoder < 0 || !avctx->codec) {
        ret = -1;
        goto fail;
    }
...
void ff_rfps_calculate(AVFormatContext *ic)
{
    ...
    for (i = 0; i < ic->nb_streams; i++) {
        ...
        st->internal->avg_frame_rate_pre = st->avg_frame_rate; //PLEX
...
int avformat_find_stream_info(AVFormatContext *ic, AVDictionary **options)
{
    ...
    ic->internal->packet_buffer_last = ic->internal->packet_buffer;
    ic->internal->packet_buffer_last_end = ic->internal->packet_buffer_end;
    ic->internal->packet_buffer = ic->internal->packet_buffer_end = NULL;
    //PLEX
...
//PLEX
        ...
        //PLEX
        if (ic->internal->packet_buffer_last)
            ret = avpriv_packet_list_get(&ic->internal->packet_buffer_last,
                                         &ic->internal->packet_buffer_last_end, &pkt1);
        else
        //PLEX
...
//PLEX
    if (ic->internal->packet_buffer_last) {
        if (ic->internal->packet_buffer)
            ic->internal->packet_buffer_end->next = ic->internal->packet_buffer_last;
        else
            ic->internal->packet_buffer = ic->internal->packet_buffer_last;
        ic->internal->packet_buffer_end = ic->internal->packet_buffer_last_end;
        ic->internal->packet_buffer_last = ic->internal->packet_buffer_last_end = NULL;
    }
//PLEX
